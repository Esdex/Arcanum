...
int ext4_dir_iterate(const ext4_fs *fs, const uint8_t *inode,
     ...
     * lookup - the common case - to speed up one that does not occur. So the honest
     * trade is to leave the reader, which every read and every path lookup goes
     * through, exactly as verified.
     */
