    ...
    Surface(modifier = Modifier.fillMaxSize(), color = MaterialTheme.colorScheme.background) {
        Column(
            ...
            Row(
                ...
                BackButton(onClick = onBack, enabled = !state.isRunning)
            ...
            if (state.error != null && !state.isRunning) {
                val errorMsg = headerOpErrorText(state.error ?: "", restore = true)
                Text(
                    text     = errorMsg,
...
@Composable
private fun RestoreLoadingContent() = OperationLoading(stringResource(R.string.restore_header_running))
